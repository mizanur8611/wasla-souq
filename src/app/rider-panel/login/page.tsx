...
export default function RiderLoginPage() {
  ...
  const [email, setEmail] = useState("rider@waslasouq.com");
  ...
  async function submit() {
    ...
    try {
      ...
      router.push("/rider-panel/dashboard");
    } catch (e: any) {
      setError(e.message);
    } finally {
      setSubmitting(false);
    ...
    <div className="flex min-h-screen items-center justify-center bg-sand px-5">
      <div className="w-full max-w-sm rounded-2xl bg-paper p-7">
        <div className="mb-6 flex items-center gap-2">
          <Bike size={22} className="text-teal" />
          <div>
            <div className="font-display text-lg font-bold text-ink">Rider App</div>
        ...
        <p className="mb-4 text-[11px] text-muted">Demo password from seed data: rider123</p>
